...
const EditModal = ({ data, onClose, onSubmit }: any) => {
  ...
  const handleError = () => {
    if (
      ...
      input?.confirm &&
      (input?.password || data?.password) == input?.confirm
    ) {
      onSubmit(input);
    } else {
      setError(true);
  ...
  return (
    <div
      ref={ref}
      className="p-[30px] bg-white md:min-w-[500px] w-full text-center rounded-[20px]"
    >
      <div className="m-auto">
        <p className="headline1 text-primary1 my-[20px]">Edit User</p>
        <div className="flex w-full">
          ...
          <div className="w-full">
            <Input
              disabled
              className="mb-[10px] w-full bg-neu4 cursor-not-allowed"
              placeHolder="ID"
              value={input?.id || data?.id}
            ></Input>
            <Input
              error={error}
              ...
              value={
                input?.username !== undefined ? input?.username : data?.username
              }
            ></Input>
            <Input
              error={error}
              ...
              value={
                input?.password !== undefined ? input?.password : data?.password
              }
            ></Input>
            <Input
              error={
                error && (input?.password || data?.password) !== input?.confirm
              }
            ...
            ></Input>
            <select
              onChange={(e: any) => getInput("role", e)}
              className="w-full border-[1px] border-black rounded-[8px] p-[8px]"
            >
              {roleList.map((item) => (
                <option selected={item.slug === data?.role?.toLowerCase()}>
                  {item.name}
                </option>
              ))}
            </select>
        ...
        <div className="flex items-center justify-center mt-[29px] md:body4 caption">
          <Button
            onClick={handleError}
            className="bg-primary1 text-white md:mr-[10px] mr-[5px]"
          >
            Save
          </Button>
